...
{
	int c[4];
	printf("%d%d%d%d" , s1[0] ,s1[1],s1[2],s1[3]);
	printf("%d%d%d%d" , s2[0] ,s2[1],s2[2],s2[3]);
	printf("\n");
	int d=0,i;
	for(i=3;i>=0;i--) {
		d =d/10 + s1[i] + s2[i];
...
int main()
{
	...
	m=0;
	if(c[0] == 'm')s1[0] = 1;
	else if(c[0] == 'c')s1[1] = 1;
	else if(c[0] == 'x')s1[2] = 1;
	else if(c[0] == 'i')s1[3] = 1;
	while(c[j]!='\0') {
		if(c[j] == ' ') {
			m=1;
			j++;
			if(c[j] == 'm'){s2[0] = 1;printf("dhruv\n");
			}
